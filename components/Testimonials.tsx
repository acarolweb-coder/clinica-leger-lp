    ...
    <section className="bg-[#FFF4EE] py-24 md:py-32 px-6 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <StaggerContainer staggerDelay={0.1} className="grid md:grid-cols-3 gap-4 md:gap-6 mb-20">
          {[1, 2, 3].map((n) => (
            <StaggerItem key={n}>
              <div className="group relative aspect-[9/16] bg-[#18191E] overflow-hidden">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-14 h-14 border border-white/20 rounded-full flex items-center justify-center group-hover:border-[#8C4821] group-hover:scale-110 transition-all duration-500">
        ...
        <ScrollReveal className="text-center mb-10">
          <span className="text-[10px] tracking-[0.4em] uppercase text-[#B6A095] font-medium">
        ...
        <StaggerContainer staggerDelay={0.1} className="grid md:grid-cols-3 gap-4 md:gap-6">
          {PRESS_ITEMS.map((item, i) => (
            <StaggerItem key={i}>
              <div className="group bg-white border border-[#B6A095]/15 p-5 hover:border-[#8C4821]/30 hover:shadow-lg hover:shadow-[#B6A095]/10 transition-all duration-500">
                {/* Outlet badge */}
                ...
                {/* Image */}
                <div className="relative aspect-[16/10] mt-3 mb-4 overflow-hidden bg-[#FFF4EE]">
                  <Image
                    src={item.image}
                    alt={item.person}
                    fill
